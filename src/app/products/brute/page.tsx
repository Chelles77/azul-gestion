  ...
  async function handleExcelImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const nouveauxProduits = jsonData.map((row: any) => {
        ...
        return { lot_id: selectedLotId, user_id: userId, nom: nom.substring(0, 100), marque, categorie, description: desc.substring(0, 200), prix_neuf: prixNeuf, coef_revient: coefBrut, prix_revient: Math.round(prixNeuf * coefBrut * 100) / 100, qr_code: generateQRCode(), statut: 'brute' as const };
      });
    ...
    } catch (err) { alert('Erreur import Excel'); } finally { setUploading(false); e.target.value = ''; }
  }

  async function creerProduitsPreRemplis() {
    if (!selectedLotId || coefBrut === 0 || !userId) return;
    const produitsToInsert = PRODUITS_INITIAUX.map(p => ({ lot_id: selectedLotId, user_id: userId, nom: p.nom, marque: p.marque, categorie: p.categorie, description: p.desc, prix_neuf: p.prixNeuf, coef_revient: coefBrut, prix_revient: Math.round(p.prixNeuf * coefBrut * 100) / 100, qr_code: generateQRCode(), statut: 'brute' as const }));
    const { error } = await supabase.from('produits').insert(produitsToInsert);
    if (!error) { fetchProduits(); alert(`${produitsToInsert.length} produits créés !`); }
  }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            <select value={selectedLotId} onChange={(e) => setSelectedLotId(e.target.value)} className="px-4 py-2 border border-gray-300 rounded-lg bg-white shadow-sm">
            ...
            <label className="px-4 py-2 bg-white border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 flex items-center gap-2 shadow-sm active:scale-[0.98] transition-all">
              <Upload size={18} className="text-gray-600" /><span className="text-gray-700 font-medium">Import Excel</span>
        ...
        <div className="bg-white rounded-xl p-6 mb-6 shadow-sm border border-gray-200 flex justify-between items-center">
          <div><span className="text-sm text-gray-500 block mb-1">Coefficient d'achat du lot</span><span className="text-3xl font-bold text-blue-600">{(coefBrut * 100).toFixed(1)}%</span></div>
          <div className="text-right"><span className="text-sm text-gray-500 block mb-1">Produits en attente</span><span className="text-3xl font-bold text-gray-900">{produits.length}</span></div>
        </div>

        <div className="flex flex-wrap gap-3 mb-6">
          <div className="relative">
            <Filter size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <select value={filterMarque} onChange={(e) => setFilterMarque(e.target.value)} className="pl-9 pr-4 py-2 border border-gray-300 rounded-lg bg-white shadow-sm">
              <option value="">Toutes marques</option>{marquesUniques.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
          <select value={filterCategorie} onChange={(e) => setFilterCategorie(e.target.value)} className="px-4 py-2 border border-gray-300 rounded-lg bg-white shadow-sm">
        ...
        {produitsFiltres.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-xl border border-gray-200 shadow-sm">
            <Package size={48} className="mx-auto text-gray-300 mb-4" /><p className="text-gray-500 text-lg">Aucun produit brut pour ce lot</p>
          </div>
        ) : (
            ...
            {produitsFiltres.map(produit => (
              <div key={produit.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-200 flex flex-col">
                <div className="h-48 bg-gray-50 relative flex items-center justify-center border-b border-gray-100">
                  {produit.photos && produit.photos.length > 0 ? <img src={produit.photos[0]} alt={produit.nom} className="w-full h-full object-cover" /> : <div className="text-gray-400 text-center"><Upload size={32} className="mx-auto mb-2" /><span className="text-sm">Ajouter photo</span></div>}
                  <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm px-2 py-1 rounded text-xs font-mono shadow-sm border border-gray-200">{produit.qrCode}</div>
                </div>
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="mb-4 mt-auto">
                    ...
                    <div className="text-xs text-gray-500 bg-gray-50 inline-block px-2 py-1 rounded">Revient : <span className="font-bold text-gray-700">{produit.prixRevient.toFixed(0)} €</span> ({(produit.coefRevient * 100).toFixed(0)}%)</div>
                  </div>
                  <div className="flex gap-2 pt-4 border-t border-gray-100">
                    <button className="flex-1 px-3 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 flex items-center justify-center gap-1 text-sm font-medium text-gray-700 active:scale-[0.98] transition-all"><Edit2 size={14} /> Modifier</button>
                    <button className="px-3 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 flex items-center justify-center active:scale-[0.98] transition-all"><QrCode size={16} className="text-gray-600" /></button>
                    <button onClick={() => mettreEnVente(produit.id)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center justify-center active:scale-[0.98] transition-all shadow-sm"><CheckCircle size={16} /></button>
                  </div>
                </div>
              </div>
            ))}
