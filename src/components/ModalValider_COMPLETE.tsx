...
export default function ModalValider({ product, isOpen, onClose, onSuccess }: ModalValiderProps) {
  ...
  const [checks, setChecks] = useState({ photo: false, etat: false, emballage: false, prix: false });
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-[#1a1a1a] border border-gray-700 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl scrollbar-hide">
        <div className="sticky top-0 bg-[#1a1a1a] border-b border-gray-800 p-6 flex justify-between items-center z-10">
          <div>
            ...
            <p className="text-sm text-green-400 font-bold mt-1">📦 Lot: {lotInfo?.numerolot || product?.lot_id || '...'}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-green-900/10 border border-green-600/30 rounded-xl p-4">
            ...
            <div className="space-y-2">
              {[
                { key: 'photo', label: checks.photo ? '✅ Photo présente' : '❌ Photo manquante' },
                { key: 'etat', label: checks.etat ? '✅ État défini' : '❌ État manquant' },
                { key: 'emballage', label: checks.emballage ? '✅ Emballage défini' : '❌ Emballage manquant' },
                { key: 'prix', label: checks.prix ? '✅ Prix défini' : '❌ Prix manquant' },
              ].map(({ key, label }) => (
                <label key={key} className={`flex items-center gap-3 p-2 rounded ${checks[key as keyof typeof checks] ? 'bg-green-600/20 border border-green-600/50' : 'bg-red-600/10 border border-red-600/30'}`}>
                  <input type="checkbox" checked={checks[key as keyof typeof checks]} disabled className="w-4 h-4" />
                  <span className={`text-sm font-medium ${checks[key as keyof typeof checks] ? 'text-green-400' : 'text-red-400'}`}>{label}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="bg-gradient-to-br from-[#252525] to-[#1a1a1a] p-6 rounded-xl border border-gray-700">
          ...
          <div className="bg-[#252525] p-6 rounded-xl border border-gray-800 flex flex-col items-center justify-center space-y-3">
            ...
            <div className="flex gap-2 w-full max-w-xs">
              <button type="button" onClick={handlePrintQR} disabled={!qrDataUrl} className="flex-1 px-3 py-2 bg-[#1a1a1a] hover:bg-[#333] border border-gray-700 text-gray-300 rounded-lg text-sm font-medium flex items-center justify-center gap-1 disabled:opacity-50">
                <Printer size={14} /> Imprimer
              </button>
              <button type="button" onClick={handleDownloadQR} disabled={!qrDataUrl} className="flex-1 px-3 py-2 bg-green-600 hover:bg-green-700 border border-green-600 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-1 disabled:opacity-50">
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-gray-700 rounded-xl p-6 text-center hover:border-green-500 transition-colors relative bg-[#252525] min-h-[150px] flex items-center justify-center">
              ...
              <input type="file" accept="image/*" onChange={handlePhotoUpload} disabled={uploading} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
            </div>
          </div>
          ...
          <div>
            ...
            <input type="text" value={nom} onChange={e => setNom(e.target.value)} className="w-full bg-[#252525] border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-green-500 outline-none" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select value={etatProduit} onChange={e => setEtatProduit(e.target.value)} className="w-full bg-[#252525] border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-green-500 outline-none">
            ...
            <div>
              ...
              <select value={etatEmballage} onChange={e => setEtatEmballage(e.target.value)} className="w-full bg-[#252525] border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-green-500 outline-none">
                <option value="">Sélectionner...</option>
                <option value="neuf">📦 Neuf</option>
                <option value="bon">👍 Bon</option>
                <option value="coupe">✂️ Coupé</option>
                <option value="abime">⚠️ Abîmé</option>
                <option value="pas_emballage">❌ Pas d'emballage</option>
              </select>
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2 flex justify-between">
              ...
              <span className="text-xs text-green-400 cursor-pointer hover:underline" onClick={() => setPrixVente(prixSuggere.toString())}>Suggérer {prixSuggere} €</span>
            </label>
            <div className="relative">
              <input type="number" value={prixVente} onChange={e => setPrixVente(e.target.value)} className="w-full bg-[#252525] border border-gray-700 rounded-lg pl-4 pr-12 py-3 text-white text-lg font-bold focus:border-green-500 outline-none" />
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <select value={selectedPlateforme} onChange={e => setSelectedPlateforme(e.target.value)} className="flex-1 bg-[#252525] border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-blue-500 outline-none">
                  <option value="">Sélectionner une plateforme...</option>
                  {PLATEFORMES_PREDEFINIES.map(p => (
                    <option key={p.id} value={p.id}>{p.emoji} {p.label}</option>
                  ))}
                </select>
                <button type="button" onClick={addPlateforme} disabled={!selectedPlateforme} className="px-4 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-700 text-white rounded-lg font-medium">+</button>
              </div>
              <div className="flex gap-2">
                <input type="text" value={customPlateforme} onChange={e => setCustomPlateforme(e.target.value)} placeholder="Ajouter une plateforme personnalisée..." className="flex-1 bg-[#252525] border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-blue-500 outline-none text-sm" onKeyPress={e => e.key === 'Enter' && addCustomPlateforme()} />
                <button type="button" onClick={addCustomPlateforme} disabled={!customPlateforme.trim()} className="px-4 py-3 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-700 text-white rounded-lg font-medium">+</button>
              </div>
              {plateformesVente.length > 0 && (
                <div className="flex flex-wrap gap-2 p-3 bg-[#252525] border border-gray-700 rounded-lg">
                  {plateformesVente.map(plateforme => (
                    <div key={plateforme} className="flex items-center gap-2 bg-blue-900/30 border border-blue-700 rounded-full px-3 py-1 text-sm text-blue-300">
                      <span>{getPlateformeLabel(plateforme)}</span>
                      <button type="button" onClick={() => removePlateforme(plateforme)} className="ml-1 text-blue-400 hover:text-blue-200">✕</button>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className="w-full bg-[#252525] border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-green-500 outline-none resize-none" />
          </div>
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-800">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-3 bg-[#252525] hover:bg-[#333] border border-gray-700 text-gray-300 font-medium rounded-lg">
              Annuler
            </button>
            <button type="submit" disabled={loading || uploading || !allChecked} className="flex-1 px-4 py-3 bg-green-600 hover:bg-green-700 disabled:bg-green-800 disabled:cursor-not-allowed text-white font-bold rounded-lg flex items-center justify-center gap-2">
              {loading ? (
                <>
